...
class Node2VecEmbeddingCreator(object):
    ...
    def create_node2vec_embeddings(graph):
        ...
        if os.path.exists('pkl/Node2Vec_embedding_new.pickle'):
            ...
            node2vec = Node2Vec(multi_gnx, dimensions=16, walk_length=30, num_walks=200, workers=1)
            model = node2vec.fit()
...
class Event2VecEmbeddingCreator(object):
    def __init__(self, graph_path, embedding_path):
        self.graph_path = graph_path
        self.embedding_path =embedding_path

    def import_dblp_graph(self):
        dblp_graph = nx.MultiGraph()
        with open(self.graph_path, 'r') as f:
            for line in f:
                items = line.strip().split()
                att1 = str(items[0][0])
                att2 = str(items[1][0])
                dblp_graph.add_node(items[0], key=att1)
                dblp_graph.add_node(items[1], key=att2)
                sort_att = np.array([att1, att2])
                sort_att = sorted(sort_att)
                dblp_graph.add_edge(items[0], items[1], key=str(sort_att[0])+str(sort_att[1]))
        return dblp_graph
...
def choose_false_edges(G, K):
    ...
    """
    if os.path.exists('pkl_e2v/dblp_non_edges_old.pickle'):
        with open('pkl_e2v/dblp_non_edges_old.pickle', 'rb') as handle:
...
def calculate_classifier_value(dict_projections, true_edges, false_edges, K):
    ...
    for edge in true_edges:
        ...
        # norm = norm = cosine_similarity(embd1.reshape(1, -1), embd2.reshape(1, -1))[0]
        X[count, 0] = norm
        Y[count, 0] = int(1)
...
def main():
    ...
    :return:
    """
    multi_graph_path = 'pkl/IMDb_multi_graph1.gpickle'
    dblp_graph_path = 'dblp/dblp.txt'
    dblp_e2v_embeddings_path = 'pkl/dblp_e2v_embeddings.txt'
    all_micro = []
    all_macro = []
    all_acc = []
    all_auc = []
    ...
    embedding_model = Node2VecEmbeddingCreator(multi_graph_path)
    # embedding_model = Event2VecEmbeddingCreator(dblp_graph_path, dblp_e2v_embeddings_path)
    # multi_graph = embedding_model.import_imdb_multi_graph()
    multi_graph = embedding_model.import_imdb_multi_graph()
    # nodes = multi_graph.nodes()
    # indexes = np.linspace(0, len(nodes)-1, 5000)
    # indexes = indexes.astype(int)
    # relevant_nodes = np.array(nodes)[indexes]
    # multi_graph = nx.subgraph(multi_graph, relevant_nodes)
    dict_node2vec_embeddings = embedding_model.create_node2vec_embeddings(multi_graph)
    ...
    true_edges = choose_true_edges(multi_graph, number)
    false_edges = choose_false_edges(multi_graph, number)
    my_dict, X, Y = calculate_classifier_value(dict_node2vec_embeddings, true_edges, false_edges, number)
    X_train, X_test, Y_train, Y_test = sk_ms.train_test_split(X, Y, test_size=ratio_arr[0])
