...
def choose_false_edges(G, K, data_name, in_prob):
    """
    ...
    if os.path.exists(os.path.join(data_name, data_path)):
        ...
        if in_prob is True:
            ...
            false_edges = []
            indexes = random.sample(range(1, len(non_edges)), 5 * K)
            for j in indexes:
...
def main():
    ...
    X_train, X_test, Y_train, Y_test = sk_ms.train_test_split(X, Y, test_size=ratio_arr[4])
