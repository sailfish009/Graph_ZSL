...
def make_false_edges(data_name, true_edges, false_per_true):
    """
    ...
    """
    data_path = data_name + '_false_edges_{}.pickle'.format(false_per_true)
    if os.path.exists(os.path.join(data_name, data_path)):
        with open(os.path.join(data_name, data_path), 'rb') as handle:
        ...
        for edge in true_edges:
            if edge[0][0] == 'c':
                ...
                label = edge[1]
                movie = edge[0]
            false_labels = list(labels - set(label))
...
def train(true_edges, false_edges, dict_embeddings, ratio, number, times, norma, data_name):
    path1 = os.path.join(data_name, 'train/classifier_e2v_l2.pkl')
    path2 = os.path.join(data_name,'train/dict_c_t_e2v_l2.pkl')
...
def evaluate(classif2, dict_c_t, true_edges, dict_embeddings, norma, data_name):
    ...
    for c in classes:
        ...
        dict_measures['acc'] = acc
        dict_class_measures[c] = dict_measures.copy()
    with open(os.path.join(data_name, 'dict_class_measures_e2v_l2.pkl'), 'wb') as handle:
...
def confision_matrix_maker(dict_class_measures, pred, pred_true):
    ...
    plt.colorbar()
    plt.savefig('our_imdb/plots/confiusion_matrix_e2v_l2')
...
def main():
    ...
    parser.add_argument('--norm', default='L2 Norm')  # cosine / L2 Norm / L1 Norm
    ...
    dict_false_edges = make_false_edges(args.data_name, true_edges, false_per_true)
    ...
    with open(os.path.join('our_imdb', 'dict_class_measures_e2v_l2.pkl'), 'rb') as handle:
